...
This approach discovered bugs that no amount of manual-reading would have caught (see [JOURNEY.md](JOURNEY.md) for the full story). The CSL is the best test suite. It exercises every major subsystem.
...
*For the B1700's hardware architecture, see [ARCHITECTURE.md](ARCHITECTURE.md). For the S-language interpreters and Gismo switching, see [INTERPRETERS.md](INTERPRETERS.md). For the development story, see [JOURNEY.md](JOURNEY.md).*
