...
struct Assembler {
    ...
    bool assemble_tokens(const std::vector<std::string>& t) {
        ...
        // ── DISPATCH LOCK|WRITE|READ ────────────────────────────────────
        if (first == "DISPATCH") {
            // I/O dispatch — emit as NOP for now (requires I/O subsystem)
            emit(0x0000);
            warn("DISPATCH not fully implemented — emitted as NOP");
            return true;
        }
    ...
    bool asm_if(const std::vector<std::string>& t) {
        ...
        // IF reg (bit) [FALSE] THEN → bit test branch
        // IF reg cmp reg [FALSE] THEN → skip when + conditional
        // IF cond GO TO label → branch

        // Check for "GO TO" pattern
        size_t goto_pos = find_token(t, "GO");
        if (goto_pos != std::string::npos) {
            size_t to_pos = find_token(t, "TO", goto_pos);
            if (to_pos != std::string::npos && to_pos + 1 < t.size()) {
                // IF ... GO TO label
                // For now, emit as unconditional branch (simplified)
                // A proper implementation would handle the condition
                warn("IF ... GO TO: condition not fully implemented, emitting conditional branch");
                return emit_branch(t[to_pos + 1], false);
            }
        }

        // IF ... THEN → emit NOP (simplified)
        // In real MIL, this would involve bit test branches (4C/5C)
        warn("IF ... THEN: simplified — emitted as NOP");
        emit(0x0000);
        return true;
    }
    ...
    bool asm_skip(const std::vector<std::string>& t) {
        // SKIP WHEN src cmp val [FALSE]
        if (t.size() < 4 || t[1] != "WHEN") {
        ...
        // 6C: skip when. MC=src_group, MD[11:10]=src_sel, MD[9:8]=variant
        // ME=test value, MF[1:0]=11 (identifies 6C)
        uint8_t variant = 1; // EQL
        uint8_t test_val = 0;

        if (cmp == "EQL") variant = 1;
        else if (cmp == "NEQ") variant = 2;
        else if (cmp == "LEQ") variant = 1; // simplified
        else if (cmp == "LSS") variant = 2; // simplified
        ...
        if (is_false) {
            // Invert: EQL→NEQ, NEQ→EQL
            variant = (variant == 1) ? 2 : 1;
        }

        uint16_t word = (reg->group << 12) | (reg->select << 10) | (variant << 8) |
                        (test_val << 4) | 0x03;
        emit(word);
        return true;
    }
